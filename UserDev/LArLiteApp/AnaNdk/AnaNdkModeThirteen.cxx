#ifndef ERTOOL_ANANDKMODETHIRTEEN_CXX
#define ERTOOL_ANANDKMODETHIRTEEN_CXX

#include "AnaNdkModeThirteen.h"

namespace ertool {

  AnaNdkModeThirteen::AnaNdkModeThirteen(const std::string& name)
  : AnaBase(name)
...

  {
    ...
    _result_tree->Branch("_n_muon", &_n_muon, "n_muon/I");
    _result_tree->Branch("_e_muon", "vector<double>", &_e_muon);
    _result_tree->Branch("_x_muon", "vector<double>", &_x_muon);
    _result_tree->Branch("_y_muon", "vector<double>", &_y_muon);
    _result_tree->Branch("_z_muon", "vector<double>", &_z_muon);
    _result_tree->Branch("_px_muon", "vector<double>", &_px_muon);
    _result_tree->Branch("_py_muon", "vector<double>", &_py_muon);
    _result_tree->Branch("_pz_muon", "vector<double>", &_pz_muon); 
    _result_tree->Branch("_n_gamma", &_n_gamma, "n_gamma/I");   
    _result_tree->Branch("_e_gamma", "vector<double>", &_e_gamma);
    _result_tree->Branch("_x_gamma", "vector<double>", &_x_gamma);
    _result_tree->Branch("_y_gamma", "vector<double>", &_y_gamma);
    _result_tree->Branch("_z_gamma", "vector<double>", &_z_gamma);
    _result_tree->Branch("_px_gamma", "vector<double>", &_px_gamma);
    _result_tree->Branch("_py_gamma", "vector<double>", &_py_gamma);
    _result_tree->Branch("_pz_gamma", "vector<double>", &_pz_gamma); 
  ...
  void AnaNdkModeThirteen::Reset()
  {}

  bool AnaNdkModeThirteen::Analyze(const EventData &data, const ParticleGraph &graph)
  {

    if (_debug)
      std::cout << "******  Begin AnaNdkModeThirteen Analysis  ******" << std::endl;
    ...
    for (auto &p : mc_graph_arr){
      // Find the muon and store its energy
      if ((p.PdgCode() == -13) && (p.Energy() > _eCut)){
        _n_muon++;
        _e_muon.push_back(p.Energy());
        _x_muon.push_back(p.Vertex()[0]);
        _y_muon.push_back(p.Vertex()[1]);
        _z_muon.push_back(p.Vertex()[2]);
        _px_muon.push_back(p.Momentum()[0]);
        _py_muon.push_back(p.Momentum()[1]);
        _pz_muon.push_back(p.Momentum()[2]);
      }

      // Find the gamma and store its energy
      if ((p.PdgCode() == 22) && (p.Energy() > _eCut)){
        _n_gamma++;
        _e_gamma.push_back(p.Energy());
        _x_gamma.push_back(p.Vertex()[0]);
        _y_gamma.push_back(p.Vertex()[1]);
        _z_gamma.push_back(p.Vertex()[2]);
        _px_gamma.push_back(p.Momentum()[0]);
        _py_gamma.push_back(p.Momentum()[1]);
        _pz_gamma.push_back(p.Momentum()[2]);
      }
    }

    // NOW LET'S DEAL WITH THE TOPOLOGY BASED SEARCH
    // fill in the below with any topological algorithm for your mode
  ...
  void AnaNdkModeThirteen::ProcessEnd(TFile* fout)
  {
  ...
  void AnaNdkModeThirteen::MakeEffPlot(std::string varname,
 ...
 void AnaNdkModeThirteen::ResetTreeVariables(){
  ...
  _n_gamma = 0;
  _e_muon.clear();
  _x_muon.clear(); _y_muon.clear(); _z_muon.clear();
  _px_muon.clear(); _py_muon.clear(); _pz_muon.clear();
  _e_gamma.clear();
  _x_gamma.clear(); _y_gamma.clear(); _z_gamma.clear();
  _px_gamma.clear(); _py_gamma.clear(); _pz_gamma.clear();
