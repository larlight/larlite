...
class wire(dataBase):
    ...
    # return
    # np.array(self._c2p.Convert(self._process.getWireData(plane,wire)))
...
class cluster(recoBase):
            ...
            for cluster in clusters:
                ...
                if self._drawParams:
                    if cluster.params().N_Hits > 10:
                        ...
                        self._listOfCParams[
                            thisPlane][-1].mouseEnter.connect(
                            self._listOfClusters[thisPlane][-1].hoverEnter)
                        self._listOfCParams[
                            thisPlane][-1].mouseExit.connect(
                            self._listOfClusters[thisPlane][-1].hoverExit)
                        self._listOfCParams[
                            thisPlane][-1].highlightChange.connect(
                            self._listOfClusters[thisPlane][-1].toggleHighlight)
                        self._listOfClusters[
                            thisPlane][-1].mouseEnter.connect(
                            self._listOfCParams[thisPlane][-1].hoverEnter)
                        self._listOfClusters[
                            thisPlane][-1].mouseExit.connect(
                            self._listOfCParams[thisPlane][-1].hoverExit)
                        self._listOfClusters[
                            thisPlane][-1].highlightChange.connect(
                            self._listOfCParams[thisPlane][-1].toggleHighlight)
    ...
    def getAutoRange(self, plane):
        w, t = self._process.getWireRange(
            plane), self._process.getTimeRange(plane)
        return [w.first, w.second], [t.first, t.second]
