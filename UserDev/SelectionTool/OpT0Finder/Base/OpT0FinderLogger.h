...
#define FLASH_DEBUG()    if( logger().debug   () ) logger().send(::flashana::msg::kDEBUG,    __FUNCTION__, __LINE__, __FILE__)
/// Compiler macro for INFO message
#define FLASH_INFO()     if( logger().info    () ) logger().send(::flashana::msg::kINFO,     __FUNCTION__, __LINE__          )
/// Compiler macro for NORMAL message
#define FLASH_NORMAL()   if( logger().normal  () ) logger().send(::flashana::msg::kNORMAL,   __FUNCTION__                    )
/// Compiler macro for WARNING message
#define FLASH_WARNING()  if( logger().warning () ) logger().send(::flashana::msg::kWARNING,  __FUNCTION__                    )
/// Compiler macro for ERROR message
#define FLASH_ERROR()    if( logger().error   () ) logger().send(::flashana::msg::kERROR,    __FUNCTION__, __LINE__          )
/// Compiler macro for CRITICAL message
#define FLASH_CRITICAL()                           logger().send(::flashana::msg::kCRITICAL, __FUNCTION__, __LINE__, __FILE__)
