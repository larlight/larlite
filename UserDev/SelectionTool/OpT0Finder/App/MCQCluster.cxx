...
namespace larlite {
	...
	MCQCluster::MCQSourceID_t MCQCluster::Identify( const unsigned int ancestor_track_id,
	        const larlite::event_mctrack& ev_mct,
	        const larlite::event_mcshower& ev_mcs) const
	{
		MCQSourceID_t res(kUnknownAncestor, -1);
		...
		for (size_t mct_index = 0; mct_index < ev_mct.size(); ++mct_index) {
			...
			if (mct.TrackID() == ancestor_track_id) {
				res.first  = kMCTrackAncestor;
				res.second = (int)mct_index;
				return res;
			}
		...
		for (size_t mcs_index = 0; mcs_index < ev_mcs.size(); ++mcs_index) {
			...
			if (mcs.TrackID() == ancestor_track_id) {
				res.first  = kMCShowerAncestor;
				res.second = (int)mcs_index;
				return res;
			}
		}
	...
	void MCQCluster::Construct( const event_mctrack& ev_mct,
	                            const event_mcshower& ev_mcs )
	{
		//
		// 0) Initialization
		//
	...
	const MCQCluster::MCQSourceID_t& MCQCluster::MCObjectID(size_t id) const
	{
