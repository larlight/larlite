...
namespace cluster{
  ...
  ClusterParamsAlg::ClusterParamsAlg(const std::vector<Hit2D> &inhitlist)
  {
    fMinNHits = 1;
    fGSer=nullptr;
    enableFANN = false;
  ...
  int ClusterParamsAlg::SetHits(const std::vector<Hit2D> &inhitlist){
  ...
  void ClusterParamsAlg::RefineStartPoints(bool override) {
    ...
    //	const std::vector<Hit2D*> whole;
    std::vector <const Hit2D*> subhit;
  ...
  void ClusterParamsAlg::FillPolygon(double frac)
  {
    ...
    if(fHitVector.size()) {
      std::vector<const Hit2D*> container_polygon;
