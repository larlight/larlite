#ifndef PROTOSHOWERALGPHOTONRECLUSTER_CXX
#define PROTOSHOWERALGPHOTONRECLUSTER_CXX

#include "ProtoShowerAlgPhotonRecluster.h"
...
namespace protoshower {
  ...
  ProtoShowerAlgPhotonRecluster::ProtoShowerAlgPhotonRecluster()
    : _params_alg(nullptr)
  {
    ...
    _name = "ProtoShowerAlgPhotonRecluster";
  }

  void ProtoShowerAlgPhotonRecluster::GenerateProtoShower(::larlite::storage_manager* storage,
							  ::larlite::event_pfpart* ev_pfpart,
							  const size_t proto_shower_pfpart,
							  protoshower::ProtoShower & proto_shower)
  {

    //std::cout << "ProtoShower start" << std::endl;
	...
	for (size_t i = 0; i < cluster_v.size(); i++) {
	  ...
	  //std::cout << "generating cluster params..." << std::endl;
	  ...
	  //std::cout << "got the cluster" << std::endl;
    ...
    // Do the vertex:
    if (_vtx_producer == "")
      std::cout << "NO VERTEX PRODUCER SPECIRIED." << std::endl;
    ...
    if (ev_vertex and (ev_vertex->size() != 0) ) {
      ...
      std::vector< ::larlite::vertex > vtx_v = { ev_vertex->at(0) };
    ...
    //std::cout << "ProtoShower done" << std::endl;
