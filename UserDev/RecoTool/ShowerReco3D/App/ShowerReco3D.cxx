...
namespace larlite {
  ...
  bool ShowerReco3D::analyze(storage_manager* storage) {
    ...
    larlite::event_ass * shower_cluster_ass_v = nullptr;
    // create association for showers -> spacepoints
    larlite::event_ass * shower_sps_ass_v = nullptr;
    // create association for showers -> vertex
    larlite::event_ass * shower_vertex_ass_v = nullptr;
    ...
    showerLikePFParts.reserve(ev_pfpart->size());
    
    unsigned int index = 0;
    for (const auto & part : *ev_pfpart) {
    ...
    auto ass_cluster_v = storage->find_one_ass(ev_pfpart->id(), ev_cluster, ev_pfpart->name());
    ...
    auto ass_vertex_v = storage->find_one_ass(ev_pfpart->id(), ev_vertex, ev_pfpart->name());
    ...
    auto ass_sps_v = storage->find_one_ass(ev_pfpart->id(), ev_sps, ev_pfpart->name());
    ...
    for (size_t i = 0; i < res_shower_v.size(); i++) {
      std::cout << "@ res_shower id=" << i << std::endl;
      ...
      if (!res_shower.fPassedReconstruction) {
	...
	std::cout << "... garbage start" << std::endl;
	continue;
      }
      if (res_shower.fDCosStart.Mag2() == 0) {
	std::cout << "... garbage angle" << std::endl;
	continue;
      }
      ...
      shower_cluster_v.push_back(ass_cluster_v.at(showerLikePFParts.at(i)));
    
      if (shower_vertex_ass_v)
	shower_vertex_v.push_back(ass_vertex_v.at(showerLikePFParts.at(i)));      

      if (shower_sps_ass_v)
	shower_sps_v.push_back(ass_sps_v.at(showerLikePFParts.at(i)));
      
      std::vector<unsigned int> pfpart_ass = { (unsigned int) showerLikePFParts.at(i) };
      shower_pfpart_v.push_back( pfpart_ass );

      std::cout << "shower_pfpart_v sz=" << shower_pfpart_v.size() << std::endl;
      
    }// for all input cluster-paris

    
    if (shower_v->empty()) {
      std::cout << "no shower, move on" << std::endl;
      return true;
    }
    ...
    shower_pfpart_ass_v->set_association(shower_v->id(),
					 product_id(data::kPFParticle,ev_pfpart->name()),
					 shower_pfpart_v);
    ...
    if ( shower_vertex_ass_v ) {
      shower_vertex_ass_v->set_association(shower_v->id(),
					   product_id(data::kVertex,ev_pfpart->name()),
					   shower_pfpart_v);
    }
    if ( shower_sps_ass_v ) {
      shower_sps_ass_v->set_association(shower_v->id(),
					product_id(data::kSpacePoint,ev_pfpart->name()),
					shower_pfpart_v);
    }
