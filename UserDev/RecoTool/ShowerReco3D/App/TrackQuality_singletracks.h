/**
 * \file TrackQuality_singletracks.h
 *
 * \ingroup ShowerReco3D
 *
 * \brief Package to run on output of shower reconstruction and make a ttree with various parameters that
 * quantify how well shower reconstruction ran. It requires reco showers and MCtracks. If you are running
 * on single particle generated files, that's all you need. If you are running on events with multiple MCtracks,
 * you also need to include simchannel files because those are needed to match a reco shower with an MCtrack.
 ...
 *
 * This is an ugly mixture of showers and tracks - I want to run the shower reconstruction over single muon events, and see the energy resolution which has to be compared with MCtrack.
 * 
 * @author kaleko
 */
...
#ifndef LARLITE_TRACKQUALITY_SINGLETRACKS_H
#define LARLITE_TRACKQUALITY_SINGLETRACKS_H
...
namespace larlite {
/**
   \class TrackQuality_singletracks
   User custom analysis class made by kazuhiro/kaleko
 */
class TrackQuality_singletracks : public ana_base {

public:

  /// Default constructor
  TrackQuality_singletracks();

  /// Default destructor
  virtual ~TrackQuality_singletracks() {};
  ...
  */
  ...
  /// Set maximum energy for MCTrack to be considered
  void SetMaxEnergyCut(const double energy) { _mc_energy_max = energy; }

  /// Set minimum energy for MCTrack to be considered
...
protected:
  ...
  /// Minimum MC track energy cut
  double _mc_energy_min;

  /// Maximum MC track energy cut
  ...
  // "cluster efficiency" is the cluster's charge divided by all hit charge associated with that mctrack
  // (IE the Q summed over all hits in the event, for single particle events)
  // TH1D *hMatchedClusterEff; ///< Matched 3D track's cluster efficiency (combined across planes)
  ...
  // TH1D *hMatchedClusterPur; ///< Matched 3D track's cluster purity (combined across planes)
  ...
  struct EventTreeParams_t {
    ...
    int n_mcshowers;
    int n_recoshowers;
    //Detprofile energy of the FIRST (for now, only) mctrack
    double mcs_E;
    double mc_containment;
  ...
  /// (only usable on single-mctrack events (single e, single gamma))
