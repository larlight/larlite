...
#ifndef LARLITE_PHOTONMERGE_CXX
#define LARLITE_PHOTONMERGE_CXX
...
namespace larlite {
  ...
  bool PhotonMerge::initialize() {

    return true;
  }
  
  bool PhotonMerge::analyze(storage_manager* storage) {
    ...
    // grab already reconstructed PFParticles
    auto ev_pfpart = storage->get_data<event_pfpart>(_pfpart_producer);
    if (_debug) { std::cout << "event has " << ev_pfpart->size() << " PFParticls" << std::endl; }
	...
	auto const& shrPoly = _shr_polygon_v;
  ...
  bool PhotonMerge::finalize() {

    return true;
  }

  // return a triangle for each plane
  void PhotonMerge::projectShower(const larlite::cluster& clus) {

    auto geom  = larutil::Geometry::GetME();
    auto geomH = larutil::GeometryHelper::GetME();

    double w2cm = geomH->WireToCm();
    double t2cm = geomH->TimeToCm();
    
    double sW = clus.StartWire() * w2cm;
    double sT = clus.StartTick() * t2cm;
    ...
    double eW = sW + _shrLen * dW;
    double eT = sT + _shrLen * dT;
    ...
    double shrWidth = _shrLen * fabs(tan(oangle)) * 0.5;
  ...
  void PhotonMerge::mergeHits(std::vector<unsigned int>& shr_hit_idx_v,
  ...
  std::vector<twodimtools::Poly2D> PhotonMerge::getPhotonPolygon(int pl) {
