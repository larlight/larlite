...
namespace showerreco{
  ...
  void dQdxModule::do_reconstruction(const ShowerClusterSet_t & inputShowers, Shower_t & resultShower){
    ...
    std::vector<double> dQ(nplanes,0);
    std::vector<double> dQQ(nplanes,0);//dQ w/ sigma selection
    std::vector<double> dx(nplanes,0);
    std::vector<double> dx_p(nplanes,0);
    std::vector<double> trunk_length(nplanes,0);
    std::vector<double> dQdx(nplanes,0);
    std::vector<double> dQdx_pitch(nplanes,0);
    std::vector<int> n_hits(nplanes,0);
    std::vector<int> n_hits_radius(nplanes,0);
    std::vector<int> n_hits_poly(nplanes,0);
    std::vector<int> n_hits_sigma(nplanes,0);
    std::vector<double> rms(nplanes,0);//charge rms
    std::vector<double> mean(nplanes,0);
    std::vector<double> sigma(nplanes,0);
    std::vector<std::vector<double> > hits_poly(nplanes,std::vector<double>());//vector of dQ/pitch on 3 cluster of 1 evt 
    std::vector<std::vector<std::vector<double> > > dQ_hit(nplanes,std::vector<std::vector<double> >());//vector of vector of dQ/pitch in one pitch on 3 clusters of all evts
    // std::vector<double> dQ_hit_clu(nplanes,0);//vector of dQ/pitch on 3 cluster of 1 evt 
