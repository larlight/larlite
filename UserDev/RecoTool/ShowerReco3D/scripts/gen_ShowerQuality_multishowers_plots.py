...
tshowername = 'fShowerQualityTree'
...
plt.savefig('%s/energy_res_multishower.png'%output_directory)
#plt.show()

#Plot of energy resolution vs energy
fig = eres_vsenergy_plot(shower_df)
plt.savefig('%s/eres_vsenergy_multishower.png'%output_directory)
#plt.show()

#2D histogram of reco vs true energy
for myplane in ['U','V','Y']:
        fig = energy_corr_plot(shower_df,plane='%s'%myplane)
        plt.savefig('%s/energy_correlation_%splane_multishower.png'%(output_directory,myplane))
        #plt.show()

#Plot of start point accuracy
fig = startpoint_acc_plot(shower_df)
plt.savefig('%s/startpoint_accuracy_multishower.png'%output_directory)
#plt.show()

#plot start-point resolution vs. energy
fig = startpoint_resolution_vsenergy_plot(shower_df)
plt.savefig('%s/startpoint_resolution_vsenergy_multishower.png'%output_directory)
#plt.show()

#Plot start point resolution per plane
fig = startpoint_resolution_plot(shower_df,fout)
plt.savefig('%s/startpoint_resolution_perplane_multishower.png'%output_directory)
#plt.show()

# 3D direction angular resolution
fig = anglediff_3d_plot(shower_df,180,fout)
plt.savefig('%s/3Ddir_accuracy_180_multishower.png'%output_directory)
#plt.show()

# 3D direction angular resolution
fig = anglediff_3d_plot(shower_df,30)
plt.savefig('%s/3Ddir_accuracy_30_multishower.png'%output_directory)
#plt.show()

# 3D angle resolution
fig = angleres_vsenergy_plot(shower_df)
plt.savefig('%s/angleres_vsenergy_plot_multishower.png'%output_directory)
#plt.show()

# 3D angle resolution vs. energy matrix
fig = angleres_vsenergy_matrix(shower_df)
plt.savefig('%s/angleres_vsenergy_matrix_multishower.png'%output_directory)
#plt.show()

# dEdx plots
fig = dEdx_plot(shower_df,fout)
plt.savefig('%s/dEdx_resolution_multishower.png'%output_directory)
#plt.show()

# dQdx plots
#fig = dQdx_plot(shower_df)
#plt.savefig('%s/dQdx_resolution_multishower.png'%output_directory)
#plt.show()

# Length plot
#fig = length_plot(shower_df)
#plt.savefig('%s/reco_shower_length_multishower.png'%output_directory)
#plt.show()
...
#plt.savefig('%s/cluster_efficiency_perplane_multishower.png'%output_directory)
#plt.show()
