...
plt.savefig('%s/eres_vsenergy.png'%output_directory)
plt.show()
...
plt.savefig('%s/startpoint_resolution_vsenergy.png'%output_directory)
plt.show()

#Plot start point resolution per plane
fig = startpoint_resolution_plot(shower_df)
plt.savefig('%s/startpoint_resolution_perplane.png'%output_directory)
plt.show()

# 3D direction angular resolution
fig = anglediff_3d_plot(shower_df,180)
plt.savefig('%s/3Ddir_accuracy_180.png'%output_directory)
plt.show()

# 3D direction angular resolution
fig = anglediff_3d_plot(shower_df,30)
plt.savefig('%s/3Ddir_accuracy_30.png'%output_directory)
plt.show()

# 3D angle resolution
fig = angleres_vsenergy_plot(shower_df)
plt.savefig('%s/angleres_vsenergy_plot.png'%output_directory)
plt.show()
...
plt.savefig('%s/dQdx_resolution.png'%output_directory)
plt.show()

# Length plot
fig = length_plot(shower_df)
plt.savefig('%s/reco_shower_length.png'%output_directory)
plt.show()

# Length vs reco energy
fig = length_energy_corr_plot(shower_df,plane='Y')
plt.savefig('%s/length_energy_correlation_Yplane.png'%(output_directory))
plt.show()

# reco efficiency
fig = reco_efficiency_vsenergy_singleshower_plot(event_df)
plt.savefig('%s/reco_efficiency_singleshower_vsenergy.png'%output_directory)
plt.show()

# reco efficiency for good angle showers
fig = reco_efficiency_vsenergy_goodangle_plot(shower_df)
plt.savefig('%s/reco_efficiency_vsenergy_goodangle.png'%output_directory)
plt.show()
