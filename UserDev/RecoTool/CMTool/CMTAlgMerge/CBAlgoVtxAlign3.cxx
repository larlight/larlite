#ifndef RECOTOOL_CBALGOVTXALIGN3_CXX
#define RECOTOOL_CBALGOVTXALIGN3_CXX

#include "CBAlgoVtxAlign3.h"
#include <map>
#include <algorithm>
...
namespace cmtool {
  ...
  //----------------------------------------
  CBAlgoVtxAlign3::CBAlgoVtxAlign3() : CBoolAlgoBase()
  //----------------------------------------
  {
  ...
  std::vector< std::vector<size_t> > CBAlgoVtxAlign3::Merge(const std::vector<cluster::Cluster>& clus_v) {
	...
	plane_clus_idx_v.push_back( plane_clus_pairs_v[i].second );
      ...
      std::pair< size_t, std::pair<float, double> > par_00;
      par_00 = std::make_pair(0,std::make_pair(_inv_float, 0));
	...
	if (_verbose){
	  std::cout << "New cluster @ Plane " << pl
		    ...
		    << "Current par00 angle : " << par_00.second.second << std::endl;
	}
	...
	if ( (clus.first == par_00.first) ) continue;
	...
	if (_verbose)
	  std::cout << "new cluster with " << cluster.size() << " hits"
		    ...
		    << "\t angle small : " << angle_small << std::endl;
  ...
  float CBAlgoVtxAlign3::ClusterDistance(const cluster::Cluster& c1,
