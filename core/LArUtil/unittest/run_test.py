...
nbad = 0
for n in range(len(poslines)):
    lpos = poslines[n].strip().split()
    pos = [ float(lpos[x]) for x in range(len(lpos)) ]
    #print pos
    
    loff = offlines[n].strip().split()
    off = [ float(loff[x]) for x in range(len(loff)) ]
    #print off

    offsets = sce.GetPosOffsets( pos[0], pos[1], pos[2] )

    diff  = [ offsets.at(x)-off[x] for x in range(len(off)) ]
    #print diff
    for d in diff:
        if d>1.0e-6:
            nbad += 1
